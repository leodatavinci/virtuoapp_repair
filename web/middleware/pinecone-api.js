/*
  ...
  Handlers combine application data from pinecone-db.js with helpers to merge the Shopify GraphQL Admin API data.
...
export default function applyQrCodeApiEndpoints(app) {
  ...
  app.post('/api/call_function', (req, res) => {
    console.log("calling the python funrction");

    const { arg1, arg2 } = req.body;

    exec(`python my_script.py ${arg1} ${arg2}`, (error, stdout, stderr) => {
      if (error) {
  ...
  app.post("/api/qrcodes", async (req, res) => {
    console.log("Flag 2")
    try {
      const id = await PineconeDB.create({
  ...
  app.patch("/api/qrcodes/:id", async (req, res) => {
    ...
    if (qrcode) {
      try {
        await PineconeDB.update(req.params.id, await parseQrCodeBody(req));
        const response = await formatQrCodeResponse(req, res, [
          await PineconeDB.read(req.params.id),
        ]);
  ...
  app.get("/api/qrcodes", async (req, res) => {
    try {
      const rawCodeData = await PineconeDB.list(
  ...
  app.delete("/api/qrcodes/:id", async (req, res) => {
    ...
    if (qrcode) {
      await PineconeDB.delete(req.params.id);
      res.status(200).send();
    }
  });
}
